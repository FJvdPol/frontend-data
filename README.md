...
I was looking for a way to easliy render a map on a website, and [timruiterkamp](https://github.com/timruiterkamp/) recommended me to try out [mapbox](https://mapbox.com).   
It was really easy to set up and within an hour I had a live world map on my screen.

I searched around for ways to combine [d3.js](https://d3js.org/) for connecting data with dom elements and mapbox for the background map. I soon found some [examples](https://github.com/jorditost/mapboxgl-d3-playground) by [jorditost](https://github.com/jorditost).
...
When it was time to show dynamic information on the screen based on what map point you focused on, I decided to quickly add [vue](https://vuejs.org) to easily render information changes.

It was quite simple to single out the data I needed per city. With the data I wanted to draw pie charts showing the ratio of values of that city compared to other cities, or the ratio of publishers within a city.
...
The bar chart was hard to read for some cities with a lot of publishers, so I decided to add a fullscreen option.   
This was really annoying to try to scale to mobile (while still being useful), so I decided to only render the pie chart on mobile instead.   
